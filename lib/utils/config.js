...
async function parse(scriptName, scriptData, configData, params) {
    ...
    if (!script.reqConfig) {
        return getResult(config, parseParams(script.args, params));
    }

    // Script requires a type and there is one, return the data
    if ((script.reqType && data.type) || (!script.reqType && !script.hasSystem)) {
        return getResult(parseConfig(config, data), parseParams(script.args, params));
    }
    ...
    if (script.hasSystem) {
        ...
        if ((script.reqType && subdata.type) || (!script.reqType && subdata)) {
            return getResult(parseConfig(config, subdata, data.all, system), parseArgs(script.args, args));
        }
    }
...
function getResult(config, args) {
    const env = {
        ...
        isProd   : config.isPreapp || args.prod   || config.system === "prod",
    };
