...
async function run() {
    ...
    Output.line();
    printTime("Timer started at", parseDate(start));
    ...
    const stopTimer = () => {
        Input.confirm("Stop the timer").then((stop) => {
            if (stop) {
                ...
                printTime("Timer stoped at", parseDate(start, time));
                startTimer();
            } else {
                stopTimer();
            }
    ...
    const startTimer = () => {
        Input.choice("Select an Option", [
            ...
            switch (answer) {
            case "continue timer":
                printTime("Timer continued at", parseDate(start, time));
                runTimer();
                break;
            case "restart timer":
                start = new Date();
                time  = 0;
                printTime("Timer restarted at", parseDate(start, time));
                runTimer();
                break;
            case "end timer":
                printTime("Timer ended at", parseDate(start, time));
 ...
 * @returns {String}
 */
function parseDate(date, seconds = 0) {
