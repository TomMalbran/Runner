...
function run(config, args, env) {
    ...
    case "flutter": {
        ...
        if (args.web) {
            let device = "chrome";
            if (args.device) {
                if (args.device.includes(":")) {
                    const [ hostname, port ] = args.device.split(":");
                    device = `web-server --web-hostname ${hostname} --web-port ${port}`;
                } else {
                    device = `web-server --web-hostname localhost --web-port ${args.device}`;
                }
            }
            Command.execChild(`flutter run -t lib/main/${file}Web.dart -d ${device}`);
        } else {
