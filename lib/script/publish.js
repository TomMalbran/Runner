...
function publishFlutter(config, args, platform) {
    showTitle(config, args, "Flutter", platform);
    Output.date();
    Output.line();
    ...
    case "ios": {
        ...
        Command.exec(`xcrun altool --upload-app -f release/${config.name}.ipa -u ${config.user} -p ${config.pass} -t ios`);
        Output.line();
...
function publishCordova(config, args, platform) {
    showTitle(config, args, "Cordova", platform);
    Output.date();
    Output.line();
 ...
 * @returns {Void}
 */
function showTitle(config, args, title, platform) {
    if (config.dev || args.dev) {
        Output.subtitle(`Publishing ${title} to ${platform}-dev`);
    } else if (config.stage || args.stage) {
        Output.subtitle(`Publishing ${title} to ${platform}-stage`);
    } else if (config.preapp || args.preapp) {
