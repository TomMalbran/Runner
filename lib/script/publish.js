...
function publishCordova(config, args, platform) {
    ...
    const type = args.tage ? "--stage" : "--prod";
    ...
    case "android": {
        Output.step("Building the Android App");
        Output.execChild("cordova build android --release --buildConfig");
        Output.move("platforms/android/app/build/outputs/apk/release/app-release.apk", `release/${config.name}.apk`);
    ...
    case "ios": {
        Output.step("Building the iOS App");
        Output.execChild("cordova build ios --release --device --buildConfig");
        Output.move("platforms/ios/build/device/$SITE_APP.ipa", `release/${config.name}.ipa`);
