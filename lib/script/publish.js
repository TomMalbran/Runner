...
function publishFlutter(config, args, env, platform) {
    ...
    case "android": {
        ...
        if (args.bundle) {
            Command.execChild(`flutter build appbundle --release -t lib/main/${file}.dart --target-platform android-arm,android-arm64,android-x64 --build-name=${config.version} --build-number=${config.build}`);
        } else {
        ...
        if (args.bundle) {
            Command.move(`${config.path}/build/app/outputs/bundle/release/app-release.aab`, `${config.path}/release/${config.name}.aab`);
        } else {
            Command.move(`${config.path}/build/app/outputs/flutter-apk/app-armeabi-v7a-release.apk`, `${config.path}/release/${config.name}-armeabi-v7a.apk`);
            Command.move(`${config.path}/build/app/outputs/flutter-apk/app-arm64-v8a-release.apk`,   `${config.path}/release/${config.name}-arm64-v8a.apk`);
            Command.move(`${config.path}/build/app/outputs/flutter-apk/app-x86_64-release.apk`,      `${config.path}/release/${config.name}-x86_64.apk`);
        }
