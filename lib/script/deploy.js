...
async function run(config, args) {
    if (config.dev || args.dev) {
        ...
        Output.subtitle("Deploying to Preapp");
    } else if (config.prod || args.prod) {
...
function upload(uconfig, config, args) {
    ...
    case "react": {
        ...
        const env = Config.getReactEnv(uconfig, args);
    ...
    case "flutter": {
        Output.step(`Building the ${uconfig.name}`);
        const file = Config.getFlutterFile(uconfig);
        Command.pushdIf(uconfig);
