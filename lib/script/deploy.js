...
function upload(uploadConfig, config, env) {
    ...
    case "react": {
        ...
        if (FS.existsSync(`${from}/build/static`)) {
            Command.ftp(config, "-e", `${from}/build/static`, `${to}/static`);
        }
        if (FS.existsSync(`${from}/build/assets`)) {
            Command.ftp(config, "-e", `${from}/build/assets`, `${to}/assets`);
        }
        Output.line();
        break;
    }
    ...
    case "flutter": {
        ...
        Command.pushdIf(uploadConfig);
        Command.exec(`flutter build web -t lib/main/${file}Web.dart --source-maps`);
        Command.popdIf(uploadConfig);
        Output.line();
        ...
        Command.ftp(config, "-e", `${from}/build/web/splash`, `${to}/splash`);
        Output.line();
        break;
    }
    ...
    case "site": {
        ...
        Output.step(`Uploading the ${uploadConfig.name}`);
        const flags = uploadConfig.delete ? "-e" : "";
        Command.ftp(config, uploadConfig.flags || flags, from, to);
        Output.line();
        break;
    }
