...
function saveChange(change, version, build) {
    ...
    switch (change.type) {
    case "file":
        Command.replace(ov, nv, Path.resolve(change.path, change.file));
        break;
    case "env":
        Command.replace(ov, nv, Path.resolve(change.path, ".env"));
        break;
    case "node":
        Command.replace(ov, nv, Path.resolve(change.path, "package.json"));
        Command.replace(ov, nv, Path.resolve(change.path, "package-lock.json"));
        break;
    case "flutter":
        Command.replace(ov,  nv,  Path.resolve(change.path, "lib", "data", "AppConfig.dart"));
        Command.replace(ovp, nvp, Path.resolve(change.path, "pubspec.yaml"));
