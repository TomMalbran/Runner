...
function run(config) {
    ...
    case "dashboard": {
        ...
        Command.copy("-f", `${fromPath}/src/Dashboard.jsx`, toPath);
        break;
    }

    case "editor": {
        Output.subtitle("Updating the Editor");
        const directories = [ "Components", "Data", "Elements", "Entities", "NLS", "Utils", "Styles" ];
    ...
    case "framework": {
        ...
        const directories = [ "config", "data", "src" ];
        const files       = [ "composer.json", ".env.example", "phpstan-rules.neon" ];
        const fromPath    = `${config.libraryDir}/Framework`;
        const toPath      = `${config.currentDir}/${config.path}/vendor/tommalbran/framework`;

        for (const file of files) {
            Command.copy("-f", `${fromPath}/${file}`, toPath);
        }
        for (const dir of directories) {
            Command.sync("-avh --delete", `${fromPath}/${dir}`, toPath, "/src/Schema", "/src/**/Schema", "/src/System", "Tester.php");
        }
        break;
    }
